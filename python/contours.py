...
#    thresh = cv2.adaptiveThreshold(imgray,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY,21,0)
    ...
    contour = cv2.approxPolyDP(label_contours[0], 10, True)
...
#        [np.sin(angle), np.cos(angle), center[1]]])
    ...
    cv2.warpAffine(thresh, rotation_matrix, dsize=(len(thresh), len(thresh[0])), dst=thresh)
    ...
    if show:
        ...
        cv2.drawContours(im, [], -1, (random.randint(0,255),random.randint(0,255),random.randint(0,255)), -5)
...
def processAllImages(dirpath):
    for file in os.listdir(dirpath):
        if file.endswith(".jpg"):
            contour(os.path.join(dirpath, file), show=True)
